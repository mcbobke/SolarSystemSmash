...
public class MoonPlayer : MonoBehaviour
{
    ...
    private void greenPlanetKill()
    {
        healthBarSliderMoon.value = 0.0f;
    }

    private void bluePlanetDamage()
    {
        healthBarSliderMoon.value -= 0.2f;
    }
    ...
    private void OnCollisionEnter2D(Collision2D col)
    {
        ...
        if ((col.gameObject.name == "Projectile(Clone)" || col.gameObject.name == "Planet1Projectile(Clone)") &&
            healthBarSliderMoon.value > 0)
        {
            applyDamage();
